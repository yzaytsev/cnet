...
jag_get_cache_sets(cjag_config_t *config) {
    ...
    for (int i = 0; i < n_pages; ++i) {
        ...
        for (int j = 0; j < 512; ++j) {
            ...
            /* printf("i=%d, j=%d, slice=%d, set=%d, cache_slices=%d, cache_ways=%d, n_pages=%d\n", i, j, slice, set, config->cache_slices, config->cache_ways, n_pages); */
            cache_set[i][set][slice][cache_set_counter[set][slice]++] = cur_addr + sub_set_offset;
        }
    }

    printf("cache_slices=%d, cache_ways=%d, n_pages=%d, n_addr_per_set=%d\n", config->cache_slices, config->cache_ways, n_pages, n_addr_per_set);
